...
source("./wrapper_SM/functions_wrapper/run_imputation_dataset.R")
source("./wrapper_SM/functions_wrapper/onset_count.R")
...
for (scheme in 2:5) {  
  results[[scheme]] <- lapply(1:100, function(seed) { 
    data <- dataset_all_SM_2K[[seed]][[scheme]]  
    n_pats <- length(unique(data$patient_id))  
    gt_onset <- sum(dataset_all_SM_2K[[seed]][[1]]$onset) / n_pats  
    scheme_onset <- data %>%
      group_by(patient_id) %>%
