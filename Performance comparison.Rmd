...
ggplot(df, aes(x = model, y = power_category, color = scenario)) +
...
```{r}
df_all$scenario <- factor(df_all$scenario, levels = c("irregular", "3-6 years", "3 years", "1 year"))

ggplot(df_all, aes(x = model, y = power_category, fill = power_category), group = interaction(model, covariate)) +
  geom_bar(stat = "identity", color = "black", aes(y=1)) +  # Aggiungi il bordo nero per le barre
 scale_fill_manual(
    values = c(
