plot_bias_baseline <- function (data, scheme, titles){
  ...
  df_long_mean<- df %>%
    pivot_longer(cols = c("rate_mean", "shape_mean"),
                 names_to = "parameter",
                 values_to = "bias_mean") %>% 
  ...
  df_long_upper<- df %>%
    pivot_longer(cols = c("rate_upper_ci", "shape_upper_ci"),
                 names_to = "parameter",
  ...
  df_long_lower<- df %>%
    pivot_longer(cols = c("rate_lower_ci", "shape_lower_ci"),
                 names_to = "parameter",
  ...
  baseline_plot <- ggplot(data = df_long %>% 
    ...
    theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1), strip.text.y = element_text(angle = 0), ) +
